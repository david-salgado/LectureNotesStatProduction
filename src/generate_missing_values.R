...
#' @param nonresponse Nonresponse type: "total" or "partial". 
...
generate_missing_values <- function(
    ...
    stopifnot(
      ...
      all(auxiliary_vars %in% names(complete_data.dt)),
      nonresponse %in% c("total", "partial"),
#      length(target_vars) == length(prop_missing),
  ...
  if (nonresponse == "total"){
    ...
    if (mechanism == "MAR") {
      ...
      prob_vector <- rep(prop_missing, length = n)
      ...
      if (mechanism_type == "linear") {
        ...
        prob_vector <- plogis(scale(linear_pred)) * prop_missing * 2
        prob_vector <- pmin(pmax(prob_vector, 0.01), 0.99)
      ...
      if (mechanism_type == "nonlinear") {
        ...
        prob_vector <- plogis(scale(effects)) * prop_missing * 2
        prob_vector <- pmin(pmax(prob_vector, 0.01), 0.99)  
      }
      ...
      if (mechanism_type == "logistic") {
        ...
        synth_y <- rbinom(nrow(complete_data.dt), 1, prop_missing)
        ...
        # Fit logistic model
        model <- glm(as.formula(frm), data = model_data, family = binomial())
          
        # Predict probabilities
        prob_vector <- predict(model, type = "response")
        prob_vector <- pmin(pmax(prob_vector, 0.01), 0.99)  
      }
      ...
      if (mechanism == "randomforest") {
        ...
        # 1. Generar variable indicadora objetivo sintética
        ...
        prob_vector <- predict(rf_model, data = model_data)$predictions[, 2]
        prob_vector <- pmin(pmax(prob_vector, 0.01), 0.99)
    ...
    if (mechanism == "MAR") {
      ...
      if (mechanism_type == "nonlinear") {
        ...
        for (i in seq_along(target_vars)) {
          ...
          # Scale to [0,1] and multiply by target-specific proportion
          prob_matrix[, i] <- plogis(scale(effects)) * prop_missing[i] * 2
          cat("ok.\n")
