...
target_estim_nonResponse.lst <- lapply(1:n_iter, function(i){
  ...
  target_yGREG_x1 <- 
    microdata_HBS_household_grTruth.dt[r == 1, HTestimator(target_resid, pik)] + 
    microdata_HBS_household_grTruth.dt[, sum(target_pred)]
  
  # GREG - x2
  x2sq_HT <- microdata_HBS_household_grTruth.dt[
    r == 1, HTestimator(get(aux_vars[2]) * get(aux_vars[2]), pik)]
  x2y_HT <- microdata_HBS_household_grTruth.dt[
    r == 1, HTestimator(get(aux_vars[2]) * get(target_vars), pik)]
  ...
  x1x2_HT <- microdata_HBS_household_grTruth.dt[
    r == 1, HTestimator(get(aux_vars[1]) * get(aux_vars[2]), pik)]
  ...
  # calib - linear - x1
  pik_sample <- microdata_HBS_household_grTruth.dt[r == 1, pik] 
  aux_vars_calib <- c(aux_vars[1], "N")
  gCalib <-  calib(
    Xs = microdata_HBS_household_grTruth.dt[r == 1, ..aux_vars_calib], 
    d  = 1/pik_sample, 
    total = aux_vars_population[c(1, 3)], method = 'linear')
  wCalib <- gCalib / pik_sample
  target_yCalib_x1 <- sum(microdata_HBS_household_grTruth.dt[r == 1, get(target_vars)] * wCalib)
  ...
  wCalib <- gCalib / pik_sample
  target_yCalib_x2 <- sum(microdata_HBS_household_grTruth.dt[r == 1, get(target_vars)] * wCalib)
  ...
  wCalib <- gCalib / pik_sample
  target_yCalib_x1x2 <- sum(microdata_HBS_household_grTruth.dt[r == 1, get(target_vars)] * wCalib)
